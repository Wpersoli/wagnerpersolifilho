...
// Canal serverless de contato via Resend, com validação, anti-abuso e logs estruturados.
...
async function sendWithResend(data) {
  ...
  if (!apiKey) return { ok: false, status: 503, code: 'provider_not_configured' };
  ...
  try {
    ...
    if (!response.ok) return { ok: false, status: response.status, code: 'provider_error' };
    return { ok: true, status: 200, id: body && body.id ? String(body.id) : '' };
  } catch (error) {
    return { ok: false, status: error && error.name === 'AbortError' ? 504 : 502, code: 'provider_unavailable' };
  } finally {
    clearTimeout(timer);
  }
}
...
async function handler(req, res) {
  ...
  http.log('info', 'contact_sent', { requestId: reqId, providerId: result.id || '' });
